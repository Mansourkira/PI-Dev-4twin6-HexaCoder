...
var url2 =
  "mongodb+srv://ahmed:ahmed@cluster0.iaanx.mongodb.net/BackendServer?retryWrites=true&w=majority";
...
class Block {
  ...
  calculateHash() {
    return SHA256(
      this.previousHash + this.timestamp + this.transactions + this.nonce
    ).toString();
  }
  getprevioushash() {
  ...
  voteblock
    .aggregate(
      [
        {
          $group: {
            _id: { $arrayElemAt: ["$transactions", 1] },
            total: { $sum: 1 },
          },
        },
      ],
...
Route.get("/stats3", async (req, res) => {
  ...
  Projects.aggregate([
    {
      $group: {
        _id: { option: "$project_option", qualified: "$state_qualified" },
        nb_qualified_project: { $sum: 1 },
      },
    },
  ])
    .match({ "_id.qualified": true })
    .sort({ nb_qualified_project: -1 })
    .exec((err, docs) => {
      if (err) {
        console.log(err);
...
Route.get("/stats4/:option", async (req, res) => {
  ...
  voteblock
    ...
    .match({ "_id.project_option": option })
    .project({ "_id.project_option": 1, "_id.team.team_Name": 1, count: 1 })
    .exec((err, docs) => {
      if (err) {
        console.log(err);
...
Route.post("/addvotes", async (req, res) => {
  ...
  voteblock.find({}, function (err, docs1) {
    ...
    const x = docs1.filter(
      (x) => x.transactions[0] == req.body.transactions[0]
    );
