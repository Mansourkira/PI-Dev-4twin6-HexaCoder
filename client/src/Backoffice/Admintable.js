...
export default function AdminTable()
{
  ...
  const [projects, setProjects] = useState([]);
   ...
   /* const handleSubmit3 = text  => event => {
      ...
      const handleSubmit3 = id  => event => {
       console.log(id[0])
       axios.get('http://localhost:8095/projects/sendmail/'+id[0]).then(res => {
        console.log(res); 
        console.log(res.data);
      })   
        ...
        const handleAnalyze = (e, id) => {
          axios.get('http://localhost:8095/analyze',{
             params :{
              github : id,
            }
          })
        ...
        const handleShow = (e, id) => {
          axios.get('http://localhost:8095/ShowAnalyze',{
             params :{
              github : id,
            }
          })
              ...
              {projects.map(project => (
                    ...
                    <tr>
                      <td>{project.project_name}</td>
                        ...
                        <td>{project.Team.map(e=>(<td>{e.team_Name}</td>))}</td>
                        <td>{project.project_option}</td>
                        
                        <td>{project.state_qualified.toString()}</td>
                        <td><a href={project.project_link}>Link to project</a></td> 
                        <td><button onClick={(e)=> handleAnalyze(e, project.project_link)} className="btn btn-primary">Show Analyze</button></td>
                        <td><button onClick={(e)=> handleShow(e, project.project_link)} className="btn btn-success">Analyze</button></td>
